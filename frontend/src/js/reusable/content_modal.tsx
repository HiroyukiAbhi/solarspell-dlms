...
> {
  update_state: (
    update_func: (draft: ContentModalState) => void,
  ) => Promise<void>;
  ...
  constructor(props: ContentModalProps) {
    ...
    this.auto_complete_filter = createFilterOptions<
  ...
  componentDidUpdate(
    prevProps: ContentModalProps,
    _prevState: ContentModalState,
  ) {
    if (
      this.props.modal_type === "edit" &&
      this.props.row !== undefined &&
      !isEqual(this.props.row, prevProps.row)
    ) {
      const row = this.props.row;
      const metadata = this.props.metadata_api.state.metadata_types.reduce(
        (prev, metadata_type) => {
          const filtered = row.metadata_info.filter((to_check) =>
            isEqual(to_check.type, metadata_type.id),
          );
      ...
      <ActionDialog
        title={
          this.props.modal_type === "add"
            ...
            : `Edit content ${this.props.row?.title}`
        }
        open={true}
        get_actions={(focus_ref) => [
          <Button key={1} onClick={this.props.on_close} color="secondary">
            Cancel
          </Button>,
          <Button
            key={2}
            onClick={() => {
              ...
              this.update_state((draft) => {
                ...
                draft.fields.content_file.value =
                  typeof file_raw === "undefined" ? null : file_raw;
              })
                .then(() =>
                  this.update_state((draft) => {
                    Object.keys(this.state.fields).map((field) => {
                      ...
                      draft.fields[cast_field].reason = this.props.validators[
                        cast_field
                ...
                .then(() => {
                  for (const key in this.state.fields) {
                    if (
                      this.state.fields[key as keyof content_fields].reason !==
                      ""
                    ) {
                  ...
                  formData.append(
                    "display_title",
                    this.state.fields.display_title.value,
                  );
                  formData.append(
                    "description",
                  ...
                  formData.append(
                    "duplicatable",
                    this.state.fields.duplicatable.value ? "true" : "false",
                  );
                  formData.append(
                    "rights_statement",
                    this.state.fields.rights_statement.value,
                  );
                  formData.append(
                    "additional_notes",
                    this.state.fields.additional_notes.value,
                  );
                  formData.append(
                    "copyright_notes",
                    this.state.fields.copyright_notes.value,
                  );
                  if (this.state.fields.year.value !== "") {
                    formData.append(
                      "published_date",
                      `${this.state.fields.year.value}-01-01`,
                    );
                  }
                  if (this.props.modal_type === "add") {
                    ...
                    formData.append(
                      "active",
                      this.state.fields.active.value ? "true" : "false",
                    );
                  }
                  if (!isNull(this.state.fields.reviewed_on.value)) {
                    formData.append(
                      "reviewed_on",
                      format(this.state.fields.reviewed_on.value, "yyyy-MM-dd"),
                    );
                  }
                  formData.append("modified_on",format(new Date(), "yyyy-MM-dd HH:mm:ss"));
                  this.props.metadata_api.state.metadata_types.map((type) => {
                    if (type.name in this.state.fields.metadata.value) {
                      this.state.fields.metadata.value[type.name].map(
                        (metadata) => {
                  ...
                  axios_response.then(
                    ...
                    (reason: any) => {
                      this.props.remove_loader();
                      const unknown_err_str =
                        this.props.modal_type === "add"
                          ? "Error while adding content"
                          : "Error while editing content";
          ...
          {[
            ...
            <>
              {this.props.row?.content_file ? (
                <Typography>
                  Existing file: {this.props.row?.file_name}
                </Typography>
              ) : null}
              <input
                accept="*"
            ...
            this.props.metadata_api.state.metadata_types.map(
              (metadata_type: SerializedMetadataType, idx) => {
                return (
                  <Grid item key={idx}>
                    <Autocomplete
                      multiple
                      value={
                        this.state.fields.metadata.value[metadata_type.name]
                      }
                      onChange={(_evt, value: SerializedMetadata[]) => {
                        ...
                        if (add_meta_tokens.length > 0) {
                          ...
                          this.props.metadata_api
                            .add_metadata(to_add.name, metadata_type)
                            .then((res) => {
                          ...
                          this.update_state((draft) => {
                            draft.fields.metadata.value[metadata_type.name] =
                              valid_meta;
                          });
                        }
                      }}
                      filterOptions={(options, params) => {
                        ...
                        const already_loaded_metadata =
                          this.props.metadata_api.state.metadata_by_type[
                            metadata_type.name
                      ...
                      handleHomeEndKeys
                      options={
                        this.props.metadata_api.state.autocomplete_metadata[
                        metadata_type.name
                        ] || []
                      }
                      ...
                      onInputChange={(_, name) => {
                        this.props.metadata_api.update_autocomplete(
                          metadata_type,
                          name,
                        );
                      }}
